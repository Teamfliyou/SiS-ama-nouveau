...
export default function Teachers() {
  ...
  const handleDelete = async (t: Teacher) => {
    ...
    setDeletingId(t.id);
    try {
      const res = await authFetch(`/api/teachers/${t.id}`, { method: 'DELETE' });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) { toast.error(data.error || 'Erreur lors de la suppression'); return; }
      toast.success('Professeur supprimé');
      if (detailTeacher?.id === t.id) setDetailTeacher(null);
      fetchTeachers();
    } catch {
      toast.error('Erreur de connexion au serveur');
    } finally {
            ...
            return (
              <div key={t.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 hover:shadow-md transition-shadow group relative">
                ...
                <div className="absolute top-4 right-4 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => setDetailTeacher(t)}
                    className="p-1.5 border border-slate-200 text-slate-400 rounded-lg hover:text-primary hover:bg-blue-50 transition-colors"
                  ...
                  <button onClick={() => handleDelete(t)} disabled={deletingId === t.id}
                    className="p-1.5 border border-slate-200 text-slate-400 rounded-lg hover:text-red-600 hover:bg-red-50 transition-colors disabled:opacity-50"
                    title="Supprimer">
                    {deletingId === t.id
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-bold text-slate-900 text-lg">
                    {detailTeacher.firstName} <span className="uppercase">{detailTeacher.lastName}</span>
                  </p>
                  <p className="text-sm text-slate-400">
                    Inscrit le {new Date(detailTeacher.createdAt).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}
                  </p>
                </div>
              </div>
              ...
              {detailTeacher.subject && (
                <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-slate-700">{detailTeacher.subject}</p>
                  </div>
                </div>
              )}
              <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl">
                ...
                <div>
                  <p className="text-xs font-semibold text-slate-400 uppercase">Classe assignée</p>
                  <p className="text-sm font-semibold text-slate-700">{detailTeacher.class?.name || 'Aucune classe'}</p>
                </div>
              </div>
              <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl">
